...
async function fetchApiDataTFT(summonerName) {
    // Check if the data exists in the cache
    const cacheKey = `tft/summoner:${summonerName}`;
    ...
    const summoner = await getSummonerId(summonerName);
    const summonerID = summoner.id;

    // Fetch data from the API if it's not in the cache
    const apiResponse = await axios.get(
        `https://euw1.api.riotgames.com/tft/league/v1/entries/by-summoner/${summonerID}?api_key=${config.apiKey}`
    );
